...
namespace carl::descriptor
{
    ...
    public:
            ...
            gsl::span<const NumberT> tuning)
        {
            ...
            DynamicTimeWarping::Match<const WristRotation<Handedness>, decltype(distanceFunction), NumberT, true, decltype(rowsCallback)>(target, query, distanceFunction, 0, std::numeric_limits<NumberT>::max(), rowsCallback);
            return results;
        }
