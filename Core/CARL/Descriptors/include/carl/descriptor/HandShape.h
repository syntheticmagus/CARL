    ...
    public:
            ...
            gsl::span<const NumberT> tuning)
        {
            ...
            for (size_t idx = 0; idx < DEFAULT_TUNING.size(); ++idx)
            {
                ...
                DynamicTimeWarping::Match<const HandShape<Handedness>, decltype(distanceFunction), NumberT, true, decltype(rowsCallback)>(target, query, distanceFunction, 0, std::numeric_limits<NumberT>::max(), rowsCallback);
            }
            return results;
        }
